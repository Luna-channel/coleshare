...
export function CharacterEditor({ character, onClose }: CharacterEditorProps) {
  ...
  useEffect(() => {
    const fetchStoryBooks = async () => {
      try {
        ...
        // 如果有角色数据，检查是否有相关的故事书
        if (character && character.stories && Array.isArray(character.stories)) {
          console.log("角色关联的故事书:", character.stories);
          
          // 从故事书路径中提取ID（这里可能需要根据实际情况调整）
          const storyIds = character.stories.map((story: any) => {
            // 尝试从故事书路径中提取ID
            const storyBook = data.contents?.find((book: any) => 
              book.blob_url === story.path || path.basename(book.blob_url) === path.basename(story.path)
            );
            return storyBook ? String(storyBook.id) : null;
          }).filter(Boolean);
          
          console.log("设置已选择的故事书IDs:", storyIds);
          setSelectedStoryBooks(storyIds);
        }
      } catch (err) {
  ...
  const handleStoryBookChange = (storyBookId: string) => {
    ...
    setSelectedStoryBooks(prev => {
      const newSelection = prev.includes(storyBookId)
        ? prev.filter(id => id !== storyBookId)
        : [...prev, storyBookId];
  ...
  const generateAndDownloadPng = async () => {
    ...
    try {
      ...
      const characterMetadata = {
        ...
        personality: characterData.personality,
        scenario: characterData.background,
        exampleDialogue: characterData.dialogue,
        ...
        selectedStoryBooks: selectedStoryBooks, // 添加故事书选择
      }
